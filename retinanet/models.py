...
class Retinanet(nn.Module):
    ...
    def predict(self, images: List[Tensor], amvs: List[Tensor]) -> List[Dict[str, Tensor]]:
        """
        ...
        images, targets1 = self.transform(images, targets)
        feature_maps1    = self.backbone(images.tensors)
        amvs, targets2 = self.transform(amvs, targets)
        feature_maps2    = self.backbone(amvs.tensors)
        feature_maps = feature_maps1
        for i in range(0,len(feature_maps1)):
            feature_maps[i] = torch.cat((feature_maps1[i],feature_maps2[i]),1)
    ...
    def forward(self, images: List[Tensor], amvs: List[Tensor], targets: Optional[List[Dict[str, Tensor]]]) -> Dict[str, Tensor]:
        """
        ...
        # store losses
        losses = {}
        losses = self.compute_loss(targets1, outputs, anchors)
        return losses
