...
def convert_annotations_to_df(annotation_dir, image_dir, image_set="train"):
    ...
    xml_df["filename2"] = [
        os.path.join(image_dir, xml_df["filename2"][i]) for i in range(len(xml_df))
    ]
...
class PascalDataset(Dataset):
    ...
    def __init__(self, dataframe, transforms):
        ...
        self.amv_ids = self.df["filename2"].unique()
    ...
    def __getitem__(self, index: int):
        ...
        amv_id = self.amv_ids[index]
        im = cv2.cvtColor(cv2.imread(image_id), cv2.COLOR_BGR2RGB)
        im2 = cv2.cvtColor(cv2.imread(amv_id), cv2.COLOR_BGR2RGB)
        # extract the bounding boxes
        records = self.df[self.df["filename"] == image_id]
        records2 = self.df[self.df["filename2"] == amv_id]
        boxes = records[["xmin", "ymin", "xmax", "ymax"]].values
        boxes2 = records2[["xmin", "ymin", "xmax", "ymax"]].values
        ...
        area2 = torch.as_tensor(area2, dtype=torch.float32)
        ...
        transformed2 = self.tfms(image=im2, bboxes=boxes2, class_labels=class_labels)
        image2 = transformed2["image"]
        boxes2 = torch.tensor(transformed2["bboxes"], dtype=torch.float32)

        class_labels = torch.tensor(transformed["class_labels"])
        class_labels2 = torch.tensor(transformed2["class_labels"])
        # target dictionary
        target = {}
        ...
        return image, image2, target, image_idx
